...
class TestHBNBCommand(unittest.TestCase):
    """Unittest the HBNB command interpreter"""
    ...
    def test_create_kwargs(self):
        with patch("sys.stdout", new=StringIO()) as test:
            self.HBNB.onecmd('create User first_name="Jolien" email="jolien@example.com password="12345"')
            new_user = test.getvalue().strip()
        with patch("sys.stdout", new=StringIO()) as test:
            self.HBNB.onecmd("all User")
            user_output = test.getvalue()
            self.assertIn(new_user, user_output)
            self.assertIn("'first_name': 'Jolien'", user_output)
            self.assertIn("'email': 'jolien@example.com'", user_output)
            self.assertNotIn("'last_name': 'carl'", user_output)
            self.assertIn("'password': '12345'", user_output)
